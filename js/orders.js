...
(function() {
    ...
    window.fetchLatestOrdersAndRender = async function() {
        ...
        try {
            ...
            // 原子化数据同步：强对齐云端，同时解析 JSON
            window.ERP_STORE.orders = (data || []).map(ord => {
                let items = ord.items;
                if (typeof items === "string") {
                    try { items = JSON.parse(items); } catch(e) { items = []; }
                } else if (ord.items_json) { // 兼容 D1 字段名
                    try { items = JSON.parse(ord.items_json); } catch(e) { items = []; }
                }
                return { ...ord, items: items || [], customer: ord.customer_name || ord.customer };
            });
    ...
    window.renderOrders = function() {
        ...
        let filteredOrders = window.ERP_STORE.orders.filter(ord => {
            ...
            let hasUnshipped = actualItems.some(item => item.status === "等待国内发货");
            let hasInWarehouse = actualItems.some(item => item.status === "集运仓已到货");
            let hasInTransit = actualItems.some(item => item.status === "跨境清关运输中");
            let hasReceived = actualItems.some(item => item.status === "买家已完成收货");
        ...
        filteredOrders.forEach((ord) => {
            ...
            let itemsSummary = (ord.items || []).map(item => {
                ...
                return `
                    <div class="flex justify-between items-center bg-slate-50/50 p-2.5 rounded-xl border border-slate-100/60 text-xs">
                        <div class="max-w-[65%] truncate">
                            ...
                            <span class="text-slate-700 font-bold ml-1">${item.name || '未命名'}</span>
                        </div>
                        <div class="flex items-center gap-2 shrink-0">
                            <span class="text-[9px] px-1.5 py-0.5 rounded-md font-black scale-90 ${subBadgeStyle}">${item.status.replace("🕒 ","").replace("📦 ","")}</span>
            ...
            const cardOpacity = isCanceled ? "opacity-65 bg-slate-50/70" : "bg-white";
            let customerName = (ord.customer || "未知").replace("CUST-CUST-", "");

            listHTML += `
                <div class="rounded-[32px] p-5 shadow-sm border border-slate-100/80 space-y-4 transition-all ${cardOpacity}">
                    <div class="flex justify-between items-start border-b border-slate-100 pb-3">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button onclick="window.openOrderDetailModalForManage(${window.ERP_STORE.orders.indexOf(ord)})" class="bg-slate-900 text-white px-3 py-1.5 rounded-xl text-xs font-black active:bg-[#5D5CDE]">
                                ${isZh ? '管理' : 'Quản lý'}
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center pt-2 text-xs">
                        <span class="text-slate-400 font-bold">${isZh?'整单内部本金':'Tổng vốn'}:</span>
        ...
        return `
            <div class="space-y-4 w-full max-w-md mx-auto pb-12">
                <button onclick="window.openOrderFormModal(null)" class="w-full bg-[#5D5CDE] text-white py-3.5 rounded-2xl text-xs font-black shadow-md flex items-center justify-center gap-1.5">
                ...
                <div class="space-y-4">${listHTML || '<div class="text-center p-10 text-slate-400">暂无数据</div>'}</div>
            </div>`;
    };
    ...
    window.openOrderDetailModalForManage = function(index) {
        ...
        let subItemsStatusRowsHTML = (ord.items || []).map((item, itemIdx) => {
            let statusBadgeClass = "bg-slate-100 text-slate-600";
            if (item.status === "集运仓已到货") statusBadgeClass = "bg-amber-50 text-amber-600 border-amber-200";
            if (item.status === "跨境清关运输中") statusBadgeClass = "bg-indigo-50 text-indigo-600 border-indigo-200";
            if (item.status === "买家已完成收货") statusBadgeClass = "bg-emerald-50 text-emerald-600 border-emerald-200";

            return `
                <div class="bg-slate-50 p-3 rounded-2xl border border-slate-200/60 space-y-3 relative">
                    <div class="flex justify-between items-start gap-2">
                        <div class="text-[11px] font-black text-slate-800 leading-tight flex-grow">
                            <span class="text-slate-400 font-bold">[${item.platform || '淘宝'}]</span> ${item.name}
                        </div>
                        <button type="button" onclick="window.toggleSingleItemStatusInModal(${index}, ${itemIdx})" class="text-[10px] font-black px-2 py-1 rounded-xl border ${statusBadgeClass} shrink-0 active:scale-95 transition-all">
                            ${item.status}
                        </button>
                    </div>
                    <div class="flex gap-1.5 items-center">
                        <div class="relative flex-grow">
                            ...
                            <input type="text" id="fast-track-input-${index}-${itemIdx}" value="${item.track || ''}" 
                                placeholder="${isZh ? '粘贴单号...' : 'Dán mã...'}" 
                                onblur="window.fastSaveSingleTrackAndCompany(${index}, ${itemIdx})"
                                class="w-full bg-white border border-slate-200 rounded-xl pl-7 pr-3 py-2 text-[11px] font-mono font-bold text-slate-700 focus:border-indigo-500 transition-all">
                        </div>
                        <select id="fast-express-select-${index}-${itemIdx}" onchange="window.fastSaveSingleTrackAndCompany(${index}, ${itemIdx})"
                            class="bg-white border border-slate-200 rounded-xl px-2 py-2 text-[10px] font-bold text-slate-600">
                            ${["中通","圆通","申通","韵达","顺丰","极兔","其他"].map(e => `<option value="${e}" ${item.express_company === e ? 'selected' : ''}>${e}</option>`).join("")}
                        </select>
                    </div>
                </div>`;
        }).join("");

        let fastButtonsConsoleHTML = ord.status !== "已取消" ? `
            <div class="space-y-2 pt-1">
                <label class="block text-[10px] text-slate-400 font-black tracking-wider uppercase">${isZh ? '⚡ 快捷全单批量更新状态' : '⚡ CẬP NHẬT NHANH'}</label>
                <div class="grid grid-cols-3 gap-2">
                    <button onclick="window.batchUpdateFullOrderStatusDirectly(${index}, '集运仓已到货')" class="bg-amber-500 text-white py-2.5 rounded-xl font-black text-xs shadow-sm active:scale-95 flex flex-col items-center">
                        <i class="fa-solid fa-warehouse text-sm"></i><span>${isZh ? '已到仓' : 'Đến kho'}</span>
                    </button>
                    <button onclick="window.batchUpdateFullOrderStatusDirectly(${index}, '跨境清关运输中')" class="bg-[#5D5CDE] text-white py-2.5 rounded-xl font-black text-xs shadow-sm active:scale-95 flex flex-col items-center">
                        <i class="fa-solid fa-truck-fast text-sm"></i><span>${isZh ? '运输中' : 'Vận chuyển'}</span>
                    </button>
                    <button onclick="window.batchUpdateFullOrderStatusDirectly(${index}, '买家已完成收货')" class="bg-emerald-600 text-white py-2.5 rounded-xl font-black text-xs shadow-sm active:scale-95 flex flex-col items-center">
                        <i class="fa-solid fa-circle-check text-sm"></i><span>${isZh ? '已签收' : 'Đã nhận'}</span>
                    </button>
                </div>
            </div>` : '';

        const modalHTML = `
            <div id="order-manage-modal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[9999] p-4">
                <div class="bg-white w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden border border-slate-100 max-h-[90vh] flex flex-col animate-fadeIn">
                    <div class="flex justify-between items-center border-b border-slate-100 p-6 shrink-0">
                        <h3 class="text-sm font-black text-slate-800">📦 订单管理控制台</h3>
                        <button onclick="document.getElementById('order-manage-modal').remove()" class="text-slate-300 text-lg">✕</button>
                    </div>
                    <div class="p-6 space-y-4 overflow-y-auto grow">
                        <div class="flex justify-between items-center bg-indigo-50/50 p-4 rounded-[28px] border border-indigo-100/50">
                            <div><div class="text-[10px] text-indigo-400 font-black uppercase">Customer</div><div class="text-lg font-black text-slate-900">${customerName}</div></div>
                            <div class="text-right"><div class="text-[10px] text-indigo-400 font-black uppercase">Order ID</div><div class="font-mono text-xs font-black text-indigo-600">${ord.id}</div></div>
                        </div>
                        <div class="space-y-3">${subItemsStatusRowsHTML}</div>
                        ${fastButtonsConsoleHTML}
                    </div>
                    <div class="p-6 border-t border-slate-100 bg-slate-50/50 flex gap-3">
                        <button onclick="window.toggleOrderCancelStatus(${index}, ${ord.status !== '已取消'})" class="w-1/3 border border-slate-200 text-slate-400 py-4 rounded-2xl text-xs font-bold">${ord.status === '已取消' ? '恢复订单' : '取消订单'}</button>
                        <button onclick="window.openOrderFormModal(${index})" class="flex-grow bg-slate-900 text-white py-4 rounded-2xl text-xs font-black shadow-md">详细编辑页面</button>
                    </div>
                </div>
            </div>`;
    ...
    // --- 4. 补全：缺失的后台逻辑函数 (严格执行保存) ---
    window.toggleSingleItemStatusInModal = function(ordIdx, itemIdx) {
        const steps = ["等待国内发货", "集运仓已到货", "跨境清关运输中", "买家已完成收货"];
        let ord = window.ERP_STORE.orders[ordIdx];
        let current = ord.items[itemIdx].status;
        ord.items[itemIdx].status = steps[(steps.indexOf(current) + 1) % steps.length];
        window.autoUpdateGlobalStatusAndSave(ordIdx);
    };

    window.fastSaveSingleTrackAndCompany = function(ordIdx, itemIdx) {
        const track = document.getElementById(`fast-track-input-${ordIdx}-${itemIdx}`).value.trim();
        const express = document.getElementById(`fast-express-select-${ordIdx}-${itemIdx}`).value;
        window.ERP_STORE.orders[ordIdx].items[itemIdx].track = track;
        window.ERP_STORE.orders[ordIdx].items[itemIdx].express_company = express;
        window.autoUpdateGlobalStatusAndSave(ordIdx);
    };

    window.batchUpdateFullOrderStatusDirectly = function(ordIdx, newStatus) {
        let ord = window.ERP_STORE.orders[ordIdx];
        ord.items.forEach(it => it.status = newStatus);
        ord.status = newStatus;
        window.autoUpdateGlobalStatusAndSave(ordIdx);
    };

    window.toggleOrderCancelStatus = function(ordIdx, shouldCancel) {
        window.ERP_STORE.orders[ordIdx].status = shouldCancel ? "已取消" : "等待国内发货";
        window.autoUpdateGlobalStatusAndSave(ordIdx);
    };

    window.autoUpdateGlobalStatusAndSave = async function(ordIdx) {
        let ord = window.ERP_STORE.orders[ordIdx];
        if (ord.status !== "已取消") {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (m) window.openOrderDetailModalForManage(ordIdx);
            }
        } catch (e) { console.error("AutoSave Error", e); }
    };

    // --- 5. 原有的编辑大表单函数 (保留你之前的全部 UI 细节) ---
